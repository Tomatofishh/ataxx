...
class GUI extends TopLevel implements View, CommandSource, Reporter {
    ...
    public void announceWin(PieceColor player) {
        //FIXME
        if (_board.redPieces() == 0) {
            showMessage("BLUE WINS!", "We have a winner", "String");
        }
        if (_board.bluePieces() == 0) {
            showMessage("RED WINS!", "We have a winner", "String");
        }
        if (_board.numJumps() >= Board.JUMP_LIMIT
        || (!_board.canMove(BLUE) && !_board.canMove(RED))) {
            if (_board.redPieces() > _board.bluePieces()) {
                showMessage("RED WINS!", "We have a winner", "String");
            } else if (_board.redPieces() == _board.bluePieces()) {
                showMessage("TIE", "End game but no one wins", "String");
            } else {
                showMessage("BLUE WINS!", "We have a winner", "String");
            }
        }
